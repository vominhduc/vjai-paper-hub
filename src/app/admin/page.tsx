...
/* ─── Reusable text input ────────────────────────────────── */
function Field({
  label, hint, value, onChange, type = "text", required = true, placeholder,
}: {
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <input
        type={type} value={value} required={required} placeholder={placeholder ?? (type === "date" ? "YYYY-MM-DD" : undefined)}
...
function TabManageCycles() {
  ...
  function buildIssueUrl() {
    ...
    const params = new URLSearchParams({
      ...
      nomination_id: action === "select" && selectNomId ? selectNomId : "",
      agenda_items:  action === "agenda" && agendaText  ? agendaText  : "",
      notes: notes || "",
    });
